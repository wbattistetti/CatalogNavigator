...
export const PHARMA_EXACT_CATEGORY_ICONS: Record<string, CategoryIconSpec> = {
  ...
  'tipo contenitore': spec('Package', CATEGORY_COLOR.anatomy),
  'materiale contenitore': spec('Layers', CATEGORY_COLOR.lab),
  'configurazione kit': spec('Link2', CATEGORY_COLOR.service),
  'dosaggio / concentrazione': spec('Scale', CATEGORY_COLOR.exam),
  'fascia di peso': spec('Scale', CATEGORY_COLOR.constraint),
...
export const PHARMA_CATEGORY_KEYWORD_RULES: Array<{ keywords: string[]; spec: CategoryIconSpec }> = [
  ...
  { keywords: ['tipo contenitore', 'confezionamento', 'blister', 'flacone', 'bustine'], spec: spec('Package', CATEGORY_COLOR.anatomy) },
  { keywords: ['materiale contenitore', 'hdpe', 'pet', 'vetro tipo'], spec: spec('Layers', CATEGORY_COLOR.lab) },
  { keywords: ['configurazione kit', 'liofil', 'solvente'], spec: spec('Link2', CATEGORY_COLOR.service) },
  { keywords: ['fascia di peso', 'peso cane', 'peso gatto'], spec: spec('Scale', CATEGORY_COLOR.constraint) },
