      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center gap-3 px-4 py-2 bg-[#050a06] border-b border-[#1a3a2a] h-10 min-w-0">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {appState === 'mainApp' && currentProject && (
            <>
              <span className="font-mono text-xs text-emerald-400/30 flex-shrink-0">—</span>
              <span className="font-mono text-xs text-emerald-400/50 uppercase tracking-wide flex-shrink-0">
                Progetto:
              </span>
              <span
                className="font-mono text-xs text-[#e8d48b]/90 truncate max-w-[280px] flex-shrink"
                title={currentProject.name}
              >
                &ldquo;{currentProject.name}&rdquo;
              </span>
              <div className="flex items-center gap-1.5 ml-3 flex-shrink-0">
                <button
                  type="button"
                  onClick={handleHome}
                  className={`${headerChip} text-[#e8d48b]/85 border-[#e8d48b]/35 hover:border-[#e8d48b]/55 hover:text-[#e8d48b] hover:bg-[#e8d48b]/8`}
                >
                  Chiudi progetto
                </button>
                <div id="project-left-actions-slot" className="flex items-center gap-1.5" />
              </div>
            </>
          )}
        </div>
        {topError && appState !== 'creatingProject' && (
          <div className="flex items-center gap-1.5 text-red-400 font-mono text-xs min-w-0 max-w-[240px]">
            <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
            <span className="truncate">{topError}</span>
          </div>
        )}
        <div id="project-toolbar-slot" className="ml-auto flex items-center gap-1.5 flex-shrink-0 min-w-0" />
      </div>
